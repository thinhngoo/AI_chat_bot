...
class BotServiceWrapper {
  ...
  Future<bool> deleteBot(String botId) async {
    if (_useImprovedService) {
      try {
        return await _improvedService.deleteBot(botId);
      } catch (e) {
        _logger.w('Error using improved deleteBot, falling back to legacy: $e');
        try {
          return await _legacyService.deleteBot(botId);
        } catch (e2) {
          _logger.e('Both bot services failed: $e2');
          rethrow;
        }
      ...
      return _legacyService.deleteBot(botId);
    }
  }
  ...
  Future<bool> importKnowledge({
    ...
    if (_useImprovedService) {
      try {
        // Check if the method exists in the improved service
        if (_improvedService is legacy.BotService) {
          return await (_improvedService as legacy.BotService).importKnowledge(
            botId: botId,
            knowledgeBaseIds: knowledgeBaseIds
          );
        } else {
          // Fallback to legacy
          _logger.w('Method not available in improved service, using legacy');
        ...
        _logger.w('Error in importKnowledge, falling back to legacy: $e');
        try {
          return await _legacyService.importKnowledge(
            botId: botId,
            knowledgeBaseIds: knowledgeBaseIds
          ...
          _logger.e('Both services failed: $e2');
          rethrow;
        }
      }
    } else {
