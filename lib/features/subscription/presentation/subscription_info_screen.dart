...
class _SubscriptionInfoScreenState extends State<SubscriptionInfoScreen> {
  ...
  Future<void> _toggleAutoRenew() async {
    ...
    try {
      ...
      final success = await _subscriptionService.toggleAutoRenewal(!currentValue);
      
      if (success && mounted) {
        _loadSubscriptionData();
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Auto-renewal ${!currentValue ? 'enabled' : 'disabled'}')),
        );
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Failed to update auto-renewal: ${e.toString()}')),
        );
      }
    }
  }
      ...
      body: _isLoading
          ...
          : _errorMessage.isNotEmpty
              ...
              : _subscription != null
                  ...
                  : const Center(child: Text('No subscription information available')),
    );
  }
